...
import { getContactBanner, getBi, IMG_URL, getCached, setCached } from "../../api/api";

const AboutBanner = () => {
  const { lang } = useLanguage();
  const cached = getCached("contactBanner");
  const [banner, setBanner] = useState(cached || null);

  useEffect(() => {
    getContactBanner().then(({ data }) => {
      if (data) {
        setBanner(data);
        setCached("contactBanner", data);
      }
    });
  }, []);

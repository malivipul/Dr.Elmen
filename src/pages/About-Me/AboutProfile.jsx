...
const AboutSection = () => {
  ...
  const label =
    getBi(profile?.label, lang) || (lang === "EN" ? "Who I Am" : "Wer ich bin");
  const title =
    getBi(profile?.title || profile?.heading, lang) ||
    (lang === "EN"
      ? "Building Smarter Organisations"
      : "Intelligentere Organisationen aufbauen");
  const rawDescription = getBi(
    profile?.description || profile?.desc || profile?.text || profile?.content,
    lang,
  );
          ...
          <div>
            ...
            <div className="space-y-4 md:space-y-5">
              {paragraphs.map((paragraph, index) => (
                <p
                  key={index}
                  className="text-[#0a3e40] text-[15px] md:text-[16px] leading-[30px] md:leading-[33px]"
                >
                  {paragraph}
                </p>
              ))}
            ...
            <div className="grid grid-cols-2 md:flex md:flex-wrap gap-4 md:gap-x-10 md:gap-y-7 mt-8 md:mt-10 pt-7 md:pt-8 border-t border-[#ddd5ca]">
              {stats.map((item, index) => (
                <div
                  key={index}
                  className="bg-white md:bg-transparent rounded-[20px] md:rounded-none border border-[#ece6dc] md:border-0 p-4 md:p-0 text-center md:text-left shadow-[0_8px_20px_rgba(0,0,0,0.03)] md:shadow-none"
                >
